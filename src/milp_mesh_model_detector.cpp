/*
 * Corresponds points in a point cloud to surfaces on a single model mesh.
 */
...
int main(int argc, char** argv) {
  ...
  if (argc != 4){
    printf("Use: milp_mesh_model_detector <numrays> <neighborhood size> <urdf>\n");
    exit(-1);
  }
  ...
  for (int i=0; i<scene_pts_tf->size(); i++){
    ...
    auto C_i = MatrixXDecisionVariable(3, 3*vertices.cols());
  ...
  prog.SetSolverOption(SolverType::kGurobi, "TimeLimit", 1200.0);
//  prog.SetSolverOption(SolverType::kGurobi, "MIPGap", 1E-12);
//  prog.SetSolverOption(SolverType::kGurobi, "Heuristics", 0.25);
//  prog.SetSolverOption(SolverType::kGurobi, "FeasRelaxBigM", 1E6);
